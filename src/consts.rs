//! This module just contains any big string literals I don't want cluttering up
...
// Regarding the loop templates: what I *want* is for the result of the closure
// to be printed to standard output *only* if it's not `()`.
//
// TODO: Merge the `LOOP_*` templates so there isn't duplicated code.  It's
// icky.

/// The template used for `--loop` input, assuming no `--count` flag is also
/// given.
...
/// When generating a package's unique ID, how many hex nibbles of the digest
/// should be used *at most*?
///
